...
function Orders() {
  ...
  return (
    <div className={`p-4 md:p-6 min-h-screen mt-10 md:mt-0 ${darkMode ? "bg-gray-900 text-white" : "bg-gray-100 text-black"}`}>
      {/* Header */}
      <div className="mb-8">
        ...
        <p className={darkMode ? "text-gray-300 text-lg" : "text-gray-600 text-lg"}>
          Track and manage all your restaurant orders efficiently.
        </p>
      </div>
      ...
      {/* Selected Orders */}
      <h2 className={`text-2xl md:text-3xl font-bold mb-6 capitalize ${darkMode ? "text-white" : "text-gray-800"}`}>
        {activeTab} Orders ({filteredOrders.length})
      </h2>

      <div className="grid gap-6 md:grid-cols-1 lg:grid-cols-2">
        {filteredOrders.length === 0 && (
          <div className={`col-span-full rounded-2xl p-8 shadow-lg text-center ${darkMode ? "bg-gray-800 text-gray-300" : "bg-white text-gray-500"}`}>
            <FiXCircle className="mx-auto text-6xl text-gray-400 mb-4" />
        ...
        {filteredOrders.map((order) => {
          ...
          return (
            ...
            >
              {/* Order Header */}
              <div className="flex justify-between items-start mb-4">
                <h3 className={darkMode ? "font-bold text-xl text-white" : "font-bold text-xl text-gray-800"}>
                  Order #{order._id.slice(-5)}
                </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div className="flex items-center">
                  ...
                  <span className={darkMode ? "text-gray-300" : "text-gray-700"}><strong>Name:</strong> {addr.fullName}</span>
                </div>
                <div className="flex items-center">
                  <FiPhone className="mr-2" />
                  <span className={darkMode ? "text-gray-300" : "text-gray-700"}><strong>Phone:</strong> {addr.phone}</span>
                </div>
                <div className="flex items-center md:col-span-2">
                  <FiMapPin className="mr-2 flex-shrink-0" />
                  <span className={darkMode ? "text-gray-300" : "text-gray-700"}><strong>Address:</strong> {addr.address}, {addr.city}</span>
                </div>
              </div>

              {/* Payment and Total */}
              <div className={`flex justify-between items-center mb-4 p-3 rounded-lg ${darkMode ? "bg-gray-700" : "bg-gray-50"}`}>
                <div className="flex items-center">
                  <FiCreditCard className="mr-2" />
                  <span className={darkMode ? "text-gray-300" : "text-gray-700"}><strong>Payment:</strong> {order.paymentType}</span>
                </div>
                <span className="text-2xl font-bold text-green-600">Rs. {order.totalPrice}</span>
              </div>
              ...
              {/* Order Items */}
              <div className="mb-4">
                <p className={darkMode ? "font-semibold text-white mb-2" : "font-semibold text-gray-800 mb-2"}>Items:</p>
                <ul className="space-y-1">
                  {order.orderItems.map((item) => (
                    <li key={item._id} className={`flex justify-between text-sm p-2 rounded ${darkMode ? "bg-gray-700 text-gray-300" : "bg-gray-50 text-gray-600"}`}>
                      <span>{item.menuitems?.name}</span>
                      <span>× {item.quantity}</span>
                    </li>
                  ))}
